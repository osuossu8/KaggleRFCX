...
class SedDatasetV6:
    def __init__(self, df, period=10, stride=5, audio_transform=None, 
                 wave_form_mix_up_ratio=None, data_path="train", mode="train"):
        self.period = period
        self.stride = stride
...
class SedDatasetV7:
    ...
    def __getitem__(self, idx):
        ...
        if self.wave_form_mix_up_ratio:
            ...
            r2 = np.random.random()
            # do mixup
            l = [i for i in range(self.len_df)]
            l.pop(idx)
            rand_idx = np.random.choice(l)
