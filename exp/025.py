...
def pretraining(fold):
    ...
    criterion = PANNsWithFocalLoss() # PANNsLoss() #BCEWithLogitsLoss() #MaskedBCEWithLogitsLoss() #BCEWithLogitsLoss()
...
for use_fold in range(5):
    with trace(f'finetune fold {use_fold}'):
        # args.pretrain_weights = f"pretrainings/EXP025/fold-{use_fold}.bin"
