...
traintpl.to_csv(OUTPUT_DIR+"folds_additional.csv", index=False)
...
def train_loop(fold):
    ...
    traintpl = pd.read_csv(OUTPUT_DIR+"folds_additional.csv")
    train_fold_add = traintpl[traintpl.kfold != fold]

    train_fold = train_df[train_df.kfold != fold]
    valid_fold = train_df[train_df.kfold == fold]

    train_fold = pd.concat([train_fold, train_fold_add], axis=0)
