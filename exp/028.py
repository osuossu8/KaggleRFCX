...
from src.datasets import SedDatasetV2, SedDatasetV3, SedDatasetV4, SedDatasetTest
...
def pretraining(fold):
    ...
    train_fold = train_additional_df[train_additional_df.kfold != fold]
    valid_fold = train_additional_df[train_additional_df.kfold == fold]
...
def main(fold):
    ...
    criterion = ClassWeightedPANNsLoss() # PANNsLoss() # FocalLoss() #BCEWithLogitsLoss() #MaskedBCEWithLogitsLoss() #BCEWithLogitsLoss()
    # optimizer = AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    num_train_steps = int(len(train_loader) * args.epochs)
    num_warmup_steps = int(0.1 * args.epochs * len(train_loader))
...
for use_fold in range(5):
    with trace(f'finetune fold {use_fold}'):
        # args.pretrain_weights = f"weights/EXP026/fold-{use_fold}.bin"
