...
def main(fold):
    ...
    train_noisy_df = pd.read_csv(args.train_noisy_csv)    
    train_noisy_df['species_id'] = train_noisy_df['pseudo_species_id'].copy()
    train_noisy_fold = train_noisy_df[train_noisy_df.kfold != fold]
    ...
    train_fold = pd.concat([train_fold, train_noisy_fold], sort=True).reset_index(drop=True)
...
class args:
    ...
    train_noisy_csv = None # CFG.TRAIN_FOLDS_NOISY_PATH
...
for use_fold in range(5):
    with trace(f'finetune fold {use_fold}'):
        args.pretrain_weights = f"best_pretrainings/SED_E0_5F_BASE/fold-{use_fold}.bin"
        args.train_noisy_csv = f"pseudo_label_fp_output/SED_E0_5F_BASE/fold-{use_fold}-pseudo_train_fp.csv"
